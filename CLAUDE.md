...
### ✅ [RESOLVED & LIVE-VALIDATED - 2026-08-11] sign-out stream recovery (was "🐛 Open bug for the next polish pass")
**NOT AN ACTIVE ITEM — do not re-propose.** Fixed and live-confirmed on 2026-08-11: signing out
mid-stream now recovers to the logon/PIN screen without a client reconnect. Everything below is
preserved as the diagnosis record only. The rules that came out of it are the load-bearing part —
ARSO locked-session tokens are not "signed in", wire-index continuity must survive a Worker
respawn, WGC never works under SYSTEM on any desktop, and the client-connected event is dead in
the split. Read those before touching `service.rs`'s upgrade path or capture-backend routing.
...
### [RESOLVED & LIVE-VALIDATED - 2026-08-11] 15.2a/b — DDA backend + live WGC↔DDA swap
**NOT AN ACTIVE ITEM — do not re-propose.** Code DONE 2026-07-09; the backend and the live swap are
live-confirmed (secure desktop visible on the client, clean `🔀 WGC → DDA` / `🔀 DDA → WGC`
transitions), and the path has been exercised continuously since. The "live validation" bullet
below is historical. Later DDA truths that supersede parts of it: a `LastPresentTime==0` acquire is
a pointer-only update and must NOT be encoded (that was the UAC-screen hover flicker), and DXGI
declines a BGRA8 request on an HDR display — so `is_hdr=false` does not mean SDR capture.
...
### [RESOLVED & LIVE-VALIDATED - 2026-08-11] 15.2c-impersonation — secure-desktop capture via thread impersonation (2026-07-10)
**NOT AN ACTIVE ITEM — do not re-propose.** The capture test passed live on 2026-07-10 and again
across the Phase 16 lock-screen work; "capture-test pending" is stale.
...
### [RESOLVED & LIVE-VALIDATED - 2026-08-11] 15.2c — thin SYSTEM launcher service (launcher plumbing)
**NOT AN ACTIVE ITEM — do not re-propose.** The service IS the deployment now and is the only way
Nova starts since Phase 16 — it supervises the Master/Worker split on every boot. The "CRITICAL
live-validation items" list below is answered: the elevated user token does NOT permit
`SetThreadDesktop(Winlogon)` (hence the SYSTEM impersonation token, 15.2c-impersonation), no extra
privileges were needed for `CreateProcessAsUserW`, and the graceful cross-process stop plus the
single-instance mutex both landed in 15.5.
...
### [RESOLVED & LIVE-VALIDATED - 2026-08-11] 15.2c installer migration — service is the default deployment
**NOT AN ACTIVE ITEM — do not re-propose.** Live since the 2026-07-13 redeploy; every install on
the dev box has gone through this path.
