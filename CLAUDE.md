...
- **AV1 (Main8/SDR) — IMPLEMENTED (2026-07-10), pending live validation.** Prior symptom: stream starts, desktop never shows, disconnect on the bitrate watchdog. Root cause was NOT packetization (GameStream's NV_VIDEO_PACKET shard/FEC format is codec-agnostic) — it was **frame-type detection**: `detect_frame_type` did HEVC NAL parsing on AV1's OBU bytes, never matched an IDR, so every frame was marked P (1) and the client never got a keyframe. Fix:
  ...
  - **Live validation needed:** stream AV1 from a Moonlight client, confirm the desktop shows + `📦 frame N ... frame_type=2 (AV1)` on the first frame (was always 1 before). AV1 Main10/HDR (`0x2000`) is NOT enabled — the shim's AV1 path is 8-bit; that's a follow-up (needs the P010/Main10 AV1 config + `from_video_format` mapping 0x2000). **NVENC AV1 encode requires RTX 40-series/Ada** — on older GPUs the AV1 session fails to init.
