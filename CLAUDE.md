...
- **AV1 does NOT work — diagnosed, NOT fixed (2026-07-10).** Symptom: stream starts, desktop never shows, client disconnects on the bitrate watchdog (`ML_ERROR_NO_VIDEO_FRAME`). Root causes (both need real work, held off to protect the working H264/HEVC paths):
  1. `ServerCodecModeSupport = 0x301` (lib.rs) omits the AV1 bit (`0x1000`), so AV1 is only half-advertised (RTSP SDP still offers `a=rtpmap:98 AV1/90000` at rtsp.rs:176). Inconsistent negotiation.
  2. **The RTP layer treats AV1 as HEVC.** `rtp_sender.set_codec(codec == Hevc || codec == Av1)` routes AV1 through HEVC **NAL-unit** framing + `detect_frame_type`/`list_nal_types` keyframe detection. AV1 has no NAL units — it uses OBUs / temporal units. So IDR detection fails (client waits forever for a keyframe) and the packetization is wrong for Moonlight's AV1 depacketizer → no decodable frame. The shim's NVENC AV1 encode (`NV_ENC_CODEC_AV1_GUID`) is set up, so this is purely a wire-format problem in `rtp.rs`.
  - **To actually enable AV1:** (a) add `0x1000` to `codec_mode_support`; (b) implement AV1 OBU-aware frame-type/keyframe detection + RTP packetization in `rtp.rs` (separate from the HEVC NAL path); (c) confirm the host GPU supports AV1 encode (NVENC AV1 needs RTX 40-series / Ada — on older GPUs the AV1 session fails and would also produce no frames). H264 and HEVC (incl. HEVC Main10/HDR10) are confirmed working and complete.
