...
- **Live validation:** (a) with no Steam/VB-CABLE sink, set `endpoint_override` to the VDD's audio endpoint name → host speakers silent during stream, audio on client; (b) start a stream and change the default output in Windows sound settings mid-stream → client-only: log shows re-assert + audio keeps flowing; host_audio: log shows rebind + audio follows; (c) confirm the pre-stream endpoint still restores on disconnect//cancel/tray-quit/sc-stop.
